...
**Best harness, best model.** OpenCode is the default agent harness (payload already installed). Claude remains via `ncl groups config update --provider claude`. Other providers are drop-in options: `/add-codex` for OpenAI's Codex (ChatGPT subscription or API key), and `/add-ollama-provider` for local open-weight models. Provider is configurable per agent group.
